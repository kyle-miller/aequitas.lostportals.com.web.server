...
public class CircleController {
	...
	@Autowired
	CircleService iconService;

	@RequestMapping(method = RequestMethod.POST)
	public ResponseEntity<Void> post(HttpServletRequest request, @RequestBody Circle icon) {
	...
	public List<Circle> getAll() {
		List<Circle> iconList = iconService.getAll();
		return iconList;
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.GET)
	public Circle get(@PathVariable String id) {
		Circle icon = iconService.get(id);
		return icon;
	}
}
